...
extension LoginCoordinator: InitialViewControllerDelegate {
    ...
    open func didSelectLogin(_ viewController: UIViewController) {
        print("pressed login")
        goToLogin()
    }
    
    open func didSelectSignup(_ viewController: UIViewController) {
        goToSignup()
    }
    
    @objc open func didSelectFacebook(_ viewController: UIViewController) {
       //Do Nothing, handle this inside application not the pods, override it
    }
